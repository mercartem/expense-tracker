...
export default function TransactionForm({...props}: ITransactionFormProps) {
  ...
  const [errors, setErrors] = useState({isValid: false, globalError: '', formErrors: {
    category: '',
    description: '',
    amount: ''
  }})
        ...
        <FormControl>
          <RadioGroup
            ...
            onChange={(e: React.ChangeEvent<HTMLInputElement>) => {
              handleChange(e, 'transactionType');
        ...
        <div className={style.block}>
          ...
          <FormControl variant='standard' sx={{ minWidth: '45%' }}>
            <TextField
              ...
              onChange={
                (e: React.ChangeEvent<HTMLInputElement>) => setFormState({...formState, amount: e.target.value.replace(/\D/g, '')})
              }
            />
          </FormControl>
        </div>
